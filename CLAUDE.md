...
- [x] **Frontend variant selection is a source edit** (found 2026-07-09 review) — fixed: variant is now selected at build time via `VITE_VARIANT` (`vite.config.js` resolves `virtual:active-app` to `src/${variant}/App.jsx`; `saviour-config`'s `build_frontend()` sets the env var), not a hardcoded import.
